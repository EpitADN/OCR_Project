...
T_Trainer* CreateTrainer_FromResource(T_Network* Network, T_TrainingResource* Resource) {
    ...
    for (int i = 0; i < nbTargets; ++i){
        if (nbEachChars == 0 || Resource->nbDuplicates[i] < nbEachChars)
            nbEachChars = Resource->nbDuplicates[i];
    }
    nbTotalChars = Resource->nbTargets * nbEachChars;
    ...
    int iChar = 0;
    for (int jD = 0; jD < nbEachChars; ++jD) {
